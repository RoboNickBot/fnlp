...
addToReport :: Ord s => Score l s -> Report l s -> Report l s
addToReport s (Report r) = Report (scoreSort (s:r))
...
compile prd = foldr addToReport mempty <$> P.toListM prd

compile' n prd = (foldr addToReport mempty . take n) <$> P.toListM prd

scores cs = P.map (getScore cs)

getScore :: CharSeq -> Candidate -> Score Language Double
getScore t = fmap (cosine (features t))
